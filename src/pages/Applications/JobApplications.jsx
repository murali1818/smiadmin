...
const JobApplications = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        ...
        const filteredApplications = response.documents.filter((app) => app.jobId === jobId); // Filter by jobId
        setApplications(filteredApplications);
        if (filteredApplications.length > 0) {
          setJobTitle(filteredApplications[0].jobtitle); // Output: Full stack developer
        } else {
          setJobTitle("No applications found.");
        }
  ...
  if (applications.lengdth === 0) return <p className="text-center text-red-500">No applications found</p>;
  ...
  {currentJobs.length > 0 && (
    <h1 className="text-2xl font-semibold text-primary flex items-start md:items-center gap-2 md:mb-0  w-full">
  ...
  <div className="flex flex-col md:flex-row lg:flex-nowrap flex-wrap gap-2 md:gap-4 lg:gap-2 w-full md:w-auto justify-end">
    {selectedApplications.length > 1 && (
      <button
        onClick={() => setShowDeleteConfirm(true)}
        className="bg-red-500 text-white px-3 py-2 rounded-md text-md hover:bg-red-600 transition flex items-center gap-2 w-full md:w-auto md:mt-0 mt-2"
      >
      ...
      {currentJobs && currentJobs.length > 0 ? (
        ...
        <h1 className="text-white text-center mt-4">Applications Not Available</h1>
      )}
